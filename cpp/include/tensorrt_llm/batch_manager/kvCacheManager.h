...
public:
    ...
    void onboardBlock(BlockPtr const& offloadBlock, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);

    //! \brief Bring block from primary to secondary memory.
    //! \details Does nothing if block is already in secondary memory.
    void offloadBlock(BlockPtr const& block, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);
...
private:
    ...
    SizeType32 loadOrAllocateBlocks(std::vector<BlockKey> const& blockKeys, SizeType32 numContextBlocks,
        GenerationRequest& sequence, std::vector<executor::RetentionPriorityAndDuration> const& perBlockRetentions, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);
    ...
    [[nodiscard]] BlockPtr getFreeBlock(
        ...
        std::optional<std::chrono::milliseconds> durationMs = std::nullopt, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);
...
public:
    ...
    //! \details Does nothing if block is already in primary memory.
    void onboardBlock(BlockPtr const& offloadBlock, SizeType32 windowSize, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);

    //! \brief Bring block from primary to secondary memory for window size.
    //! \details Does nothing if block is already in secondary memory.
    void offloadBlock(BlockPtr const& block, SizeType32 windowSize, executor::KvCacheTransferMode mode = executor::KvCacheTransferMode::DRAM, std::optional<std::string> directory = std::nullopt);
